...
export default function OfficerDashboardPage() {
  ...
  const sourceOptions = useMemo(() => {
    if (form.type === "Case") return cases.map((item) => ({ id: item.id, label: `${item.id} - ${item.name}` }));
    if (form.type === "AAR") return aars.map((item) => ({ id: item.id, label: `${item.id} - ${item.caseId}` }));
        ...
        <PageHeader
          ...
          action={
            <div className="flex gap-2">
              <button onClick={() => setOpenQuickAdd(true)} className="rounded-md bg-blue-700 px-3 py-2 text-sm font-medium text-white hover:bg-blue-800">Quick Add Knowledge</button>
              <Link href="/officer/repository/add" className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Full Knowledge Form</Link>
            </div>
          }
        />
        ...
        <Modal open={openQuickAdd} onClose={() => setOpenQuickAdd(false)} title="Quick Add Knowledge">
          <div className="space-y-3">
            ...
            <label className="block text-sm">
              ...
              <select className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" value={form.type}
                onChange={(e) => { const nextType = e.target.value as "Case" | "AAR" | "SOP"; setForm((f) => ({ ...f, type: nextType, sourceId: "" })); }}>
            ...
            <label className="block text-sm">
              ...
              <select className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" value={form.sourceId}
            ...
            <button
              onClick={() => {
                ...
                setFormError("");
                setForm({ title: "", type: "Case", sourceId: "", summary: "", description: "", createdBy: currentUser?.name ?? "Officer Hana" });
              }}
