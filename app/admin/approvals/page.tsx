...
export default function AdminApprovalsPage() {
  ...
  const selectedCase =
    selectedApproval?.type === "case" && selectedApproval.sourceId
      ? cases.find((item) => item.id === selectedApproval.sourceId)
      : null;
  const selectedAar =
    selectedApproval?.type === "aar" && selectedApproval.sourceId
      ? aars.find((item) => item.id === selectedApproval.sourceId)
      : null;
  const selectedSop =
    selectedApproval?.type === "sop" && selectedApproval.sourceId
      ? sops.find((item) => item.id === selectedApproval.sourceId)
      : null;
        ...
        render: (row: (typeof approvals)[number]) => (
          <div className="flex gap-2">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                dispatch(reviewApproval({ id: row.id, decision: "Approved", notes: notes[row.id] ?? "Approved" }));
              }}
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                dispatch(reviewApproval({ id: row.id, decision: "Rejected", notes: notes[row.id] ?? "Rejected" }));
              }}
      ...
      <AppShell role="admin">
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as "dateDesc" | "dateAsc" | "type" | "submitter")}
